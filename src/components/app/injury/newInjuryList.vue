<script setup>
import { watch, onMounted, ref, computed, provide } from 'vue';
...
const updateExporting = (newExport) => {
    // console.log(newExport);

    exporting.value = newExport;
};
...
onMounted(async () => {
    ...
    await loadList();
    // console.log('list:', injuryList.value);
...
async function getInjuryList(hardRefresh, hardRefreshStatus) {
    ...
    // console.log('reseP: ', hardRefreshStatus);
...
const loadList = async (hardRefresh) => {
    ...
    if (activePanel.value == 1) {
        ...
        injuryList.value = response.data;
        // console.log('here:', injuryList.value);
    }
    ...
    if (activePanel.value == 4) {
        ...
        // console.log('here:', forArchived.value);
        ...
        if (forArchived.value) {
            ...
            if (employeeIds.length > 0) {
                ...
                console.log('No employee IDs available for fetching names.');
            }
        } else {
...
const updateSelectedPatients = (newSelectedPatients) => {
    selectedPatients.value = newSelectedPatients;
};
...
watch(
    ...
    (newEndDate) => {
        ...
        // console.log('newEndDate: ', formattedEndDate.value);
    }
);
...
<template>
    <div class="bg">
        ...
        <div class="col-12 xl:col-12" v-if="showT">
            <div class="card shadow-4 mx-5 opacity-90" style="height: 700px">
                ...
                <TabView :lazy="false" @tab-change="updateActivePanel">
                    <TabPanel header="Injury Cases">
                        <template #header>
                            <div v-if="!injuryList && activePanel === '0'">
                                <ProgressSpinner style="width: 50px; height: 50px" strokeWidth="8" fill="var(--surface-ground)" animationDuration=".5s" aria-label="Custom ProgressSpinner" />
                            </div>
                        </template>
                        <InjuryCasesTabView :filterValue="filterValue" :list="filteredInjuryListAdmitRemoved" :loading="isLoading" :newList="true" />
                    </TabPanel>
                    <TabPanel header="Admitted  Cases">
                        <template #header>
                            <div v-if="!injuryList && activePanel === '1'">
                                <ProgressSpinner style="width: 50px; height: 50px" strokeWidth="8" fill="var(--surface-ground)" animationDuration=".5s" aria-label="Custom ProgressSpinner" />
                            </div>
                        </template>
                    ...
                    <TabPanel header="Export Admitted Cases">
                        ...
                        <ExportCasesTabView :filterValue="filterValue" :list="filteredExportList" @update:selectedPatients="updateSelectedPatients" :loading="isLoading" />
                    </TabPanel>
                    ...
                    <TabPanel header="Archived">
                        <template #header>
                            <div v-if="!groupedArchivedData && activePanel == '4'">
                                <ProgressSpinner style="width: 50px; height: 50px" strokeWidth="8" fill="var(--surface-ground)" animationDuration=".5s" aria-label="Custom ProgressSpinner" />
                            </div>
                        </template>
