    ...
    const generateText = () => {
        try {
            ...
            if (patientStore.details.ExternalCauseOfInjury.ext_bite_sp && patientStore.details.ExternalCauseOfInjury.ext_bite === 'Y') {
                ...
                //Management/Plan
                if (patientStore.progressionDay === '0' && patientStore.details.ABTC.immunization_schedule === 'PRIMARY REGIMEN' && patientStore.type_prophylaxis === 'PRE-EXPOSURE') {
                    //                     const nonPharmacologicManagement = ref('');
                    //                     if(patientStore.details.followUp.biting === 'ALIVE'){
                    // nonPharmacologicManagement.value =
                    //                     }else{

                    //                     }
                    // if (patientStore.details.ABTC.tt === 'Y' && patientStore.details.ABTC.pvrv === 'Y') {
                    //     patientStore.doctor_plan = 'Day 0: Administer the primary regimen for pre-exposure prophylaxis. \nARV 0.1 ML X ID 2 SITES.\nTT 0.5 ML IM.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                    // } else if (patientStore.details.ABTC.pvrv === 'Y' && patientStore.details.ABTC.tt !== 'Y') {
                    //     patientStore.doctor_plan =
                    //         'Day 0: Administer the primary regimen for pre-exposure prophylaxis. \nARV' + patientStore.details.ABTC.pvrv_site2 + ' ML X ID 2 SITES.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                    // } else if (patientStore.details.ABTC.pvrv !== 'Y' && patientStore.details.ABTC.tt === 'Y') {
                    //     patientStore.doctor_plan =
                    //         'Day 0: Administer the primary regimen for pre-exposure prophylaxis. \nTT ' + patientStore.details.ABTC.tt_num + ' ML IM.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                    // } else {
                    //     patientStore.doctor_plan = 'Day 0: Administer the primary regimen for pre-exposure prophylaxis. \nARV 0.1 ML X ID 2 SITES.\nTT 0.5 ML IM.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                    // }
                    const { tt, pvrv, pvrv_site2, tt_num } = patientStore.details.ABTC;

                    let doctorPlan = 'Day 0: Administer the primary regimen for pre-exposure prophylaxis. \n';

                    if (tt === 'Y' && pvrv === 'Y') {
                        doctorPlan += `ARV ${pvrv_site2} ML X ID 2 SITES.\nTT ${tt_num} ML IM.`;
                    } else if (pvrv === 'Y') {
                        doctorPlan += `ARV ${pvrv_site2} ML X ID 2 SITES.`;
                    } else if (tt === 'Y') {
                        doctorPlan += `TT ${tt_num} ML IM.`;
                    } else {
                    ...
                    patientStore.doctor_plan = doctorPlan;
                } else if (patientStore.progressionDay === '7' && patientStore.details.ABTC.immunization_schedule === 'PRIMARY REGIMEN' && patientStore.type_prophylaxis === 'PRE-EXPOSURE') {
                    const { tt, pvrv, pvrv_site2, tt_num } = patientStore.details.ABTC;

                    let doctorPlan = 'Day 7: Administer the primary regimen for pre-exposure prophylaxis. \n';
                    ...
                    patientStore.doctor_plan = doctorPlan;
                    // patientStore.doctor_plan = 'Day 7: Administer the primary regimen for pre-exposure prophylaxis. \nARV 0.1 ML X ID 2 SITES.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '21' && patientStore.details.ABTC.immunization_schedule === 'PRIMARY REGIMEN' && patientStore.type_prophylaxis === 'PRE-EXPOSURE') {
                    patientStore.doctor_plan = 'Day 21: Review pre-exposure prophylaxis status. \nARV 0.1 ML X ID 2 SITES.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '0' && patientStore.details.ABTC.immunization_schedule === 'BOOSTER' && patientStore.type_prophylaxis === 'PRE-EXPOSURE') {
                    patientStore.doctor_plan = 'Day 0: Administer the booster for pre-exposure prophylaxis. \nARV 0.1 ML X ID 1 SITE.\nTT 0.5 ML IM.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '3' && patientStore.details.ABTC.immunization_schedule === 'PRIMARY REGIMEN' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
                    patientStore.doctor_plan =
                        'Day 3: Administer the primary regimen for post-exposure prophylaxis. \nARV 0.1 ML X ID 2 SITES.\nTT 0.5 ML IM.\nNon-pharmacologic management: Daily wound care, advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '7' && patientStore.details.ABTC.immunization_schedule === 'PRIMARY REGIMEN' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
                    patientStore.doctor_plan =
                        'Day 7: Administer the primary regimen for post-exposure prophylaxis. \nARV 0.1 ML X ID 2 SITES.\nTT 0.5 ML IM.\nNon-pharmacologic management: Daily wound care, advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '28' && patientStore.details.ABTC.immunization_schedule === 'PRIMARY REGIMEN' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
                    patientStore.doctor_plan = 'Day 28: Review post-exposure prophylaxis status. \nARV 0.1 ML X ID 2 SITES.\nAdvised on foods to avoid.';
                } else if (patientStore.progressionDay === '0' && patientStore.details.ABTC.booster_regimen === '1' && patientStore.details.ABTC.immunization_schedule === 'BOOSTER' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
                    patientStore.doctor_plan =
                        'Day 0: Administer the first booster for post-exposure prophylaxis. \nARV BOOSTER 0.1 ML X ID 1 SITE.\nTT 0.5 ML IM.\nNon-pharmacologic management: Daily wound care, advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '0' && patientStore.details.ABTC.booster_regimen === '2' && patientStore.details.ABTC.immunization_schedule === 'BOOSTER' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
                    patientStore.doctor_plan =
                        'Day 0: Administer the second booster for post-exposure prophylaxis. \nARV BOOSTER 0.1 ML X ID 1 SITE.\nTT 0.5 ML IM.\nNon-pharmacologic management: Daily wound care, advised on succeeding follow up and foods to avoid.';
                } else if (patientStore.progressionDay === '3' && patientStore.details.ABTC.booster_regimen === '2' && patientStore.details.ABTC.immunization_schedule === 'BOOSTER' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
                    patientStore.doctor_plan =
                        'Day 3: Administer the second booster for post-exposure prophylaxis. \nARV BOOSTER 0.1 ML X ID 1 SITE.\nNon-pharmacologic management: Daily wound care, advised on succeeding follow up and foods to avoid.';
                }
            } else {
