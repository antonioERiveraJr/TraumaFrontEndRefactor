            ...
            if (patientStore.details.ExternalCauseOfInjury.ext_bite_sp && patientStore.details.ExternalCauseOfInjury.ext_bite === 'Y') {
                ...
                if (patientStore.progressionDay !== '') {
                    followUp.value = {
                        ...
                        '               Adverse Reaction From Previous Vaccination': patientStore.details.followUp.adverse_reaction,
                        '               Biting': patientStore.details.followUp.biting,
                        '               Others': patientStore.details.followUp.others
                    };
                }
                const filteredFollowUp = Object.fromEntries(
                    Object.entries(followUp.value).filter(([key, value]) => {
                        return value !== undefined && value !== null && value !== '' && value !== 'Invalid date';
                    })
                );
                const mapFollowUp = Object.keys(filteredFollowUp)
                    .map((label) => `${label}: ${filteredFollowUp[label]}`)
                    .join('\n');
                const filteredTally = Object.fromEntries(
                    Object.entries(tally).filter(([key, value]) => {
                        return value !== undefined && value !== null && value !== '' && value !== 'Invalid date';
