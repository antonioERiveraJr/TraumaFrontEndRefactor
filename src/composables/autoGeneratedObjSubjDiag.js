                ...
                if (patientStore.details.ExternalCauseOfInjury.ext_bite_sp) {
                    ...
                    if (patientStore.progressionDay === '0' || patientStore.progressionDay === '') {
                        tally.value = {
                            ...
                            '               Applied Anything to Wound':
                                patientStore.details.preAdmissionData.first_aid_code === 'Y' ? 'Yes' : patientStore.details.preAdmissionData.first_aid_code === 'N' ? 'No' : patientStore.details.preAdmissionData.first_aid_code,
                    ...
                    if (patientStore.details.followUp.finding === 'WITH FINDINGS') {
                        tallyFindings.value = `With Findings:\nWound Description: ${
                            patientStore.details.followUp.wound_description === 'OTHERS' ? patientStore.details.followUp.wound_description_others : patientStore.details.followUp.wound_description
                        }
...
(${patientStore.details.followUp.discharge})Discharge/Bleeding ${patientStore.details.followUp.discharge_sp}`;
                        if (patientStore.details.followUp.finding_sp) {
                            tallyFindings.value += `\n${patientStore.details.followUp.finding_sp}`;
                        }
...
(${patientStore.details.followUp.discharge})Discharge/Bleeding ${patientStore.details.followUp.discharge_sp}`;
                ...
                //Management/Plan
                const { tt, pvrv, pcec, pvrv_site_1_id, pvrv_site_1_im, pvrv_site_2, pvrv_site_4, pcec_site_1_id, pcec_site_1_im, pcec_site_2, pcec_site_4 } = patientStore.details.ABTC;
                ...
                if (patientStore.progressionDay !== '' || undefined || null) {
                    ...
                    //     if (pvrv === 'Y') {
                    //         if (pvrv_site_1_im === 'Y') {
                    //             doctorPlan += `ARV PVRV 0.5 ML`;
                    //         } else {
                    //             doctorPlan += `ARV PVRV 0.1 ML`;
                    //         }
                    //         if (pvrv_site_1_id === 'Y') {
                    //             doctorPlan += ` X ID 1 SITE.\n`;
                    //         } else if (pvrv_site_1_im === 'Y') {
                    //             doctorPlan += ` X IM 1 SITE.\n`;
                    //         } else if (pvrv_site_2 === 'Y') {
                    //             doctorPlan += ` X ID 2 SITES.\n`;
                    //         } else if (pvrv_site_4 === 'Y') {
                    //             doctorPlan += ` X ID 4 SITES.\n`;
                    //         }
                    //     } else if (pcec === 'Y') {
                    //         if (pcec_site_1_im === 'Y') {
                    //             doctorPlan += `ARV PCEC 0.5 ML`;
                    //         } else {
                    //             doctorPlan += `ARV PCEC 0.1 ML`;
                    //         }
                    //         if (pcec_site_1_id === 'Y') {
                    //             doctorPlan += ` X ID 1 SITE.\n`;
                    //         } else if (pcec_site_1_im === 'Y') {
                    //             doctorPlan += ` X IM 1 SITE.\n`;
                    //         } else if (pcec_site_2 === 'Y') {
                    //             doctorPlan += ` X ID 2 SITES.\n`;
                    //         } else if (pcec_site_4 === 'Y') {
                    //             doctorPlan += ` X ID 4 SITES.\n`;
                    //         }
                    //     }
                    ...
                    if (patientStore.type_prophylaxis === 'PRE-EXPOSURE') {
                        ...
                        let booster_plan = '';
                        // let booster_plan = 'PrEP Booster ';
                        if (patientStore.details.ABTC.immunization_schedule === 'BOOSTER') {
                            // patientStore.doctor_plan = 'Day 0: Administer the booster for pre-exposure prophylaxis. \nARV 0.1 ML X ID 1 SITE.\nTT 0.5 ML IM.\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.\nAdvised';

                            booster_plan += `PrEP Booster `;
                        } else {
                            booster_plan += `Day- ${patientStore.progressionDay} PrEP `;
                        }
                        if (pvrv === 'Y') {
                            if (pvrv_site_1_im === 'Y') {
                                booster_plan += `ARV PVRV 0.5 ML`;
                            } else {
                                booster_plan += `ARV PVRV 0.1 ML`;
                            }
                            if (pvrv_site_1_id === 'Y') {
                                booster_plan += ` X ID 1 SITE.\n`;
                            } else if (pvrv_site_1_im === 'Y') {
                                booster_plan += ` X IM 1 SITE.\n`;
                            } else if (pvrv_site_2 === 'Y') {
                                booster_plan += ` X ID 2 SITES.\n`;
                            } else if (pvrv_site_4 === 'Y') {
                                booster_plan += ` X ID 4 SITES.\n`;
                            }
                        } else if (pcec === 'Y') {
                            if (pcec_site_1_im === 'Y') {
                                booster_plan += `ARV PCEC 0.5 ML`;
                            } else {
                                booster_plan += `ARV PCEC 0.1 ML`;
                            }
                            if (pcec_site_1_id === 'Y') {
                                booster_plan += ` X ID 1 SITE.\n`;
                            } else if (pcec_site_1_im === 'Y') {
                        ...
                        if (tt === 'Y') {
                            booster_plan += `TT 0.5 ML IM.`;
                        }
                        patientStore.doctor_plan = booster_plan + '\nNon-pharmacologic management: Advised on succeeding follow up and foods to avoid.\nAdvised';
                        ...
                        const hrig_num = details.ABTC.hrig_num || '';
                        ...
                        if (progressionDay === '28') {
                            let booster_plan = 'Day 28/30: ';
                            if (pvrv === 'Y') {
                                if (pvrv_site_1_im === 'Y') {
                                    booster_plan += `ARV PVRV 0.5 ML`;
                                } else {
                        ...
                        } else if (patientStore.progressionDay !== '') {
                            let plan;

                            if (patientStore.details.ABTC.immunization_schedule === 'BOOSTER') {
                                if (patientStore.details.ABTC.booster_regimen && patientStore.details.ABTC.opted === 'Y') {
                                    plan = `PrEP Day-${patientStore.progressionDay} (${patientStore.details.ABTC.booster_regimen}) \n`;
                                } else if (patientStore.details.ABTC.booster_regimen && patientStore.details.ABTC.opted !== 'Y') {
                                    plan = `PEP Day-${patientStore.progressionDay} (${patientStore.details.ABTC.booster_regimen}) \n`;
                                }
                                if (patientStore.details.ABTC.vaccine_none_sp === 'NO ARV NEEDED' && patientStore.details.ABTC.vaccine_none === 'Y') {
                                    plan += `NO ARV NEEDED.\n`;
                                } else if (patientStore.details.ABTC.vaccine_none_sp === 'STILL COVERED' && patientStore.details.ABTC.vaccine_none === 'Y') {
                                    plan += `NO ARV NEEDED, STILL COVERED BY PREVIOUS REGIMEN.\n`;
                                } else if (patientStore.details.ABTC.vaccine_none_sp === 'NO ARV TODAY' && patientStore.details.ABTC.coveredDate && patientStore.details.ABTC.vaccine_none === 'Y') {
                                    plan += `NO ARV TODAY, TOO EARLY FOR DOSE, TO COME BACK ON ${patientStore.details.ABTC.coveredDate} FOR DAY-${patientStore.progressionDay}.\n`;
                                }
                                // if (patientStore.details.ABTC.booster_regimen === '2-DAY REGIMEN') {
                                //     plan = `PEP Day-${patientStore.progressionDay} `;
                                // }
                                if (pvrv === 'Y') {
                                    if (pvrv_site_1_im === 'Y') {
                                        plan += `ARV PVRV Booster 0.5 ML`;
                                    } else {
                                        plan += `ARV PVRV Booster 0.1 ML`;
                                    }
                                    if (pvrv_site_1_id === 'Y') {
                                        plan += ` X ID 1 SITE.\n`;
                                    } else if (pvrv_site_1_im === 'Y') {
                                    ...
                                    if (pcec_site_1_im === 'Y') {
                                        plan += `ARV PCEC Booster 0.5 ML`;
                                    } else {
                                        plan += `ARV PCEC Booster 0.1 ML`;
                                    }
                                    if (pcec_site_1_id === 'Y') {
                                        plan += ` X ID 1 SITE.\n`;
                                    } else if (pcec_site_1_im === 'Y') {
                                ...
                                if (patientStore.details.ABTC.ats === 'Y' && patientStore.details.ABTC.skintest === '-') {
                                    plan += `ATS ${ats_num} IU IM ANST\n`;
                                }
                                if (patientStore.details.ABTC.tt === 'Y') {
                                    plan += `TT 0.5 ML IM\n`;
                                }
                                // if (patientStore.details.ABTC.erig_num !== '' && patientStore.details.ABTC.erig === 'Y' && patientStore.details.ABTC.skintest === '-') {
                                //     plan += `ERIG ${erig_num} ML INFILTRATE TO AFFECTED AREA, EXCESS GIVEN DEEP IM, AFTER NEGATIVE SKIN TEST\n`;
                                // }
                                // if (patientStore.details.ABTC.hrig_num !== '' && patientStore.details.ABTC.hrig === 'Y') {
                                //     plan += `HRIG ${hrig_num} ML INFILTRATE TO AFFECTED AREA, EXCESS GIVEN DEEP IM, AFTER POSITIVE SKIN TEST\n`;
                                // }
                                // plan += `ARV BOOSTER 0.1 ML X ID ${boosterRegimen === '1' ? '4' : '1'} SITES.\nTT 0.5 ML IM.\n`;
                            } else {
                                if (patientStore.details.ABTC.opted === 'Y') {
                                    plan = `PrEP Day-${patientStore.progressionDay} \n`;
                                } else {
                                    plan = `PEP Day-${patientStore.progressionDay} `;
                                }
                                // plan = `PEP Day-${patientStore.progressionDay} `;
                                ...
                                if (pvrv === 'Y') {
                                    if (pvrv_site_1_im === 'Y') {
                                        plan += `ARV PVRV 0.5 ML`;
                                    } else {
                                        plan += `ARV PVRV 0.1 ML`;
                                    }
                                    if (pvrv_site_1_id === 'Y') {
                                        plan += ` X ID 1 SITE.\n`;
                                    } else if (pvrv_site_1_im === 'Y') {
                                    ...
                                    if (pcec_site_1_im === 'Y') {
                                        plan += `ARV PCEC 0.5 ML`;
                                    } else {
                                        plan += `ARV PCEC 0.1 ML`;
                                    }
                                    if (pcec_site_1_id === 'Y') {
                                        plan += ` X ID 1 SITE.\n`;
                                    } else if (pcec_site_1_im === 'Y') {
                            ...
                            if (patientStore.details.ABTC.medication !== '' && patientStore.patientTSSRecord?.data?.[0] && patientStore.patientTSSRecord?.data?.[0]?.data?.enccode.toLowerCase() !== localStorage.getItem('enccode').toLowerCase()) {
                                // console.log(patientStore.patientTSSRecord?.data?.[0]?.data?.enccode.toLowerCase(), localStorage.getItem('enccode').toLowerCase());
                                plan += `Continue ${patientStore.details.ABTC.medication}.\n`;
                            } else if (
                                (patientStore.details.ABTC.medication !== '' && !patientStore.patientTSSRecord?.data?.[0]) ||
                                (patientStore.patientTSSRecord?.data?.[0]?.data?.enccode.toLowerCase() === localStorage.getItem('enccode').toLowerCase() && patientStore.details.ABTC.medication !== '')
                            ) {
                                // console.log(patientStore?.patientTSSRecord?.data?.[0]?.data?.enccode.toLowerCase(), localStorage.getItem('enccode').toLowerCase());
                                plan += `Medication:  ${patientStore.details.ABTC.medication}.\n`;
                            }
            ...
            if (patientStore.type_prophylaxis === 'PRE-EXPOSURE') {
                patientStore.chief_complaint = `PrEP Day-${patientStore.progressionDay}`;
            } else if (patientStore.progressionDay === '0' && patientStore.type_prophylaxis === 'POST-EXPOSURE') {
